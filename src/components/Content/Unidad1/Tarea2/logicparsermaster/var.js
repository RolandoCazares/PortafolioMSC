import { MissingTruthValue, MissingValue } from "./errors";
...
export class LogicVar {
  ...
  constructor(value = null, hasNot = null, operatorType = null, nodeObj = null) {
    ...
    if (value == null && nodeObj == null) {
      throw new MissingValue("Required variable for LogicVar. Must use either value or nodeObj.");
    }
    ...
    // Check if nodeObj is not null
      ...
      // Try to load value
      if (!("value" in nodeObj)) {
        throw new MissingValue("Required key for LogicVar. Must have \"value\" key in nodeObj.");
      } else {
  ...
  evaluate(truthValue = {}) {
    ...
    if (!(this.getValue() in truthValue)) {
      throw new MissingTruthValue(`Required truth value for the variable "${this.getValue()}".`);
    }
