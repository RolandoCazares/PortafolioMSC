...
import { UnbalancedParentheses } from "./errors"
...
const getTruthValue = function (value, power) {
...
function parseExpression(expression, hasNot = false, operatorType = null) {
  ...
  for (const index in expression) {
    ...
    // Check for close parenthesis
      ...
      if (parentDepth === 0) {
        ...
        let exp = parseExpression(
          expression.substring(last, index),
          tempHasNot,
          operatorType
        );
    ...
    if (
      ...
      char.charCodeAt(0) !== "v".charCodeAt(0)
    ) {
  ...
  if (parentDepth !== 0) {
    throw new UnbalancedParentheses("You have a missing parenthesis somewhere.");
  }
  variables.sort();
