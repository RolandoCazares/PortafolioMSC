...
import { MissingValue } from "./errors";
import { valueIn } from "./globals"
import { LogicVar } from "./var";
...
export class LogicNode {
  ...
  constructor(operator = null, left = null, right = null, hasNot = false, operatorType = null, nodeObj = null) {
    ...
    if (operator == null && left == null && right == null && nodeObj == null) {
      throw new MissingValue("Required variable(s) for LogicNode. Must use either operator, left, and right or nodeObj.");
    }
    ...
    // Check if nodeObj exists
      ...
      if (!("operator" in nodeObj)) {
        throw new MissingValue("Required key for LogicNode. Must have \"operator\" in nodeObj.");
      } else {
      ...
      // Try loading left
      if (!("left" in nodeObj)) {
        throw new MissingValue("Required key for LogicNode. Must have \"left\" in nodeObj.");
      } else {
      ...
      // Try loading right
      if (!("right" in nodeObj)) {
        throw new MissingValue("Required key for LogicNode. Must have \"right\" in nodeObj.");
      } else {
